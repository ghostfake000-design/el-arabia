...
/**
 * تصدير البيانات إلى ملف Excel (xlsx) مهيكل واحترافي
 * @param rows مصفوفة من المصفوفات تمثل الأسطر (Array of Arrays)
 * @param filename اسم الملف المستخرج
 */
export const exportToExcel = (rows: any[][], filename: string) => {
  const wb = XLSX.utils.book_new();
  const ws = XLSX.utils.aoa_to_sheet(rows);
  
  // ضبط اتجاه الورقة ليكون من اليمين لليسار
  if(!ws['!cols']) ws['!cols'] = [];
  ws['!views'] = [{RTL: true}];
